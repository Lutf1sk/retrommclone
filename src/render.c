...
void render_draw_rect(void* usr, usz count, lt_gui_rect_t* r, u32* clrs) {
	...
	for (usz i = 0; i < count; ++i) {
		...
		glColor4ub(cr, cg, cb, ca); glVertex2f(r[i].x, r[i].y);
		glColor4ub(cr, cg, cb, ca); glVertex2f(r[i].x + r[i].w, r[i].y);
		glColor4ub(cr, cg, cb, ca); glVertex2f(r[i].x + r[i].w, r[i].y + r[i].h);
		glColor4ub(cr, cg, cb, ca); glVertex2f(r[i].x, r[i].y + r[i].h);
	}
	glEnd();
}
...
void render_draw_text(void* usr, usz count, lt_gui_point_t* pts, lstr_t* strs, u32* clrs) {
	...
	for (usz i = 0; i < count; ++i) {
		...
		while (it < end) {
			...
			glColor4ub(r, g, b, a); glTexCoord2f(beg, 0.0f); glVertex2f(x, y);
			glColor4ub(r, g, b, a); glTexCoord2f(end, 0.0f); glVertex2f(x + w, y);
			glColor4ub(r, g, b, a); glTexCoord2f(end, 1.0f); glVertex2f(x + w, y + h);
			glColor4ub(r, g, b, a); glTexCoord2f(beg, 1.0f); glVertex2f(x, y + h);
...
void render_draw_icon(void* usr, usz icon, lt_gui_rect_t* r, u32 clr) {
	...
	glBegin(GL_QUADS);
	glColor4ub(cr, cg, cb, ca); glTexCoord2f(0.0f, 0.0f); glVertex2f(r->x, r->y);
	glColor4ub(cr, cg, cb, ca); glTexCoord2f(1.0f, 0.0f); glVertex2f(r->x + r->w, r->y);
	glColor4ub(cr, cg, cb, ca); glTexCoord2f(1.0f, 1.0f); glVertex2f(r->x + r->w, r->y + r->h);
	glColor4ub(cr, cg, cb, ca); glTexCoord2f(0.0f, 1.0f); glVertex2f(r->x, r->y + r->h);
	glEnd();
